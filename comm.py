...
INIT_DB_SQL = ['''create table if not exists projects
    (
    ...
    (
      ...
      FOREIGN KEY (SchemaId) REFERENCES datasources(ID) ON delete CASCADE ON UPDATE CASCADE
    );''']
...
def getDatasourceRecords(db, datasourceID=None):
    ...
    datasources = db.exec_select(sql)
...
def insertSchemaRecord(db, schema_name, schema_driver,schema_host, schema_port, other):
    ...
    :return:
    """
    sql = '''insert into datasources VALUES (NULL,'%s','%s','%s',%d,'%s')''' % (schema_name, schema_driver,schema_host, int(schema_port), other)
    r = db.exec_statement(sql)
    return r
